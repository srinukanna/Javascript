...
## Hoisting
* It is a core JavaScript behavior where declarations of variables, functions, and classes are conceptually moved to the top of their containing scope during the compilation phase, before the code is executed. 

```
The code isn't physically moved; rather, the JavaScript engine processes the declarations in advance, allocating memory for them. 
 ```
...
* Only the variable declaration part is hoisted, not the function assignment. 
* Calling them before the assignment line will result in a TypeError (if var is used, because the value is undefined) or ReferenceError (if let/const is used, due to the TDZ).
